#!/usr/bin/env python3
"""sense5_antimix_drift.py — SENSE-5 similarity-gated AMPLITUDE anti-mixing warm Phi-drift A/B.

Sibling of sense4_repel_drift.py / sense3_hebb_drift.py. Identical warm protocol — load the REAL
warm 467-word `state/pure_mind/mind.json` READ-ONLY into a throwaway copy, beta=0 / hebb-off /
repel-off / anti-mixing-off warm-DOWN to the first Phi<=target (~31) so BOTH arms share the
IDENTICAL low-Phi start cell-state, then replay the SAME 35 captured Codex caregiver lines. The
ONLY toggled variable over the measured window:

    diff_gain = 0.0    (control, bit-exact legacy)
    diff_gain = 1.0    (treatment, similarity-gated amplitude anti-mixing — SENSE-5, sol's spec)

Both arms are beta=0 (assoc-blend OFF) AND hebb_eta=0 (SENSE-3 OFF) AND repel_gamma=0 (SENSE-4 OFF)
so the anti-mixing force is the SOLE toggled variable (the doc says this explicitly). torch/numpy/
python RNG are seeded identically BEFORE PureMind is built; the warm-down runs with diff_gain=0 in
BOTH arms, then diff_gain is flipped on the engine instance right before the 35 measured turns. The
SENSE-5 block adds NO new RNG draws, so diff_gain is the sole variable at the point it turns on.

Pre-registered measures (SENSE-5.md), all logged:
  PRIMARY    drift = Phi_end - Phi_start on the 3 down-drift seeds (1, 9999, 12345);
             success = sign flip OR >=50% shallower
  SECONDARY  paired mean Delta Phi_drift (diff1.0 - diff0.0), 10 seeds, expected >= +0.5
  STRUCTURE  `_amplitudes` SVD participation ratio PR=(sum s^2)^2/sum s^4 -> treatment PR >= 1.15
             AND mean pairwise cosine distance in 0.04-0.10, both above the paired control
  GATE       mean node_sim + collapse fraction (frac cells node_sim>0.90) — is the gate even open?
             (if node_sim < 0.90 the gate is CLOSED and that is the headline)
  REJECTION  top-singular-share s0^2/sum s^2 (winner-take-all / cold-basin mode-collapse watch)
  GUARDRAILS end-Phi < 40 (no cold-basin re-inflation) · mean frustration in [0.45, 0.55] ·
             vocab byte-identical 467 -> 467 · too_frustrated fire-rate logged (limit-cycle watch)
...
# 35 caregiver lines — verbatim from sense2/sense3/sense4 drivers (same replayed Codex turns)
CAREGIVER = [
    "아가야, 엄마가 사랑해.",
    "그래, 꽃잎은 부드럽고 향기는 달콤해.",
    "응, 나뭇잎이 살랑이고 새가 노래해.",
...
def gate_stats(engine):
    """Arm-INDEPENDENT gate readout: recompute deg-weighted mean cosine node_sim from the current
    engine state (works in BOTH arms — the control never runs the block, so we measure it here).
    Returns (mean_node_sim, collapse_fraction, mean_collapse_magnitude)."""
    import torch
    ...
    der, dec = adj.indices()
    dew = adj.values()
    deg = engine._degrees.clamp(min=1)                                 # [N]
    U = F.normalize(A, p=2, dim=1, eps=1e-8)
    edge_sim = (U[der] * U[dec]).sum(dim=1)                            # [E]
    node_sim = torch.zeros(n).index_add_(0, der, dew * edge_sim) / deg # [N]
    collapse = ((node_sim - 0.90) / 0.10).clamp(0.0, 1.0)             # same gate as step() 2b
    open_frac = float((node_sim > 0.90).float().mean())
    return float(node_sim.mean()), open_frac, float(collapse.mean())
...
def run_one(seed, diff_gain, warm, curriculum, warmup_reps, warmup_target, out_fh):
    ...
    eng.repel_gamma = 0.0                # SENSE-4 OFF in BOTH arms (not a confound)
    eng.diff_gain = 0.0                  # warm-down is anti-mixing-OFF in BOTH arms (identical start)
    ...
    # WARMUP: drive cells DOWN into the low-Phi (~30) SENSE-GROW basin (all forces off, beta=0).
    warm_phi = []
    used_reps = 0
    ...
    # ---- flip anti-mixing ON for the measured window (the SOLE variable) ----
    eng.diff_gain = diff_gain

    def snap(t):
        pr, cd, tsh = diff_metrics(eng)
        nsim, ofrac, cmag = gate_stats(eng)
        return {"turn": t, "phi": round(float(pc.phi), 4), "T": round(float(pc.tension), 4),
                ...
                "collapse_mag": round(cmag, 4)}

    traj = [snap(0)]
    out_fh.write(json.dumps({"tag": "ready", "seed": seed, "diff_gain": diff_gain,
    ...
    for t, line in enumerate(CAREGIVER, 1):
        ...
        s = snap(t)
        out_fh.write(json.dumps({"tag": "turn", "seed": seed, "diff_gain": diff_gain, "child": child, **s},
                                ensure_ascii=False) + "\n")
        traj.append(s)
    ...
    ofrac = [x["gate_open"] for x in traj]
    cmag = [x["collapse_mag"] for x in traj]
    import numpy as np
    ...
    summ = {"tag": "summary", "seed": seed, "diff_gain": diff_gain,
            ...
            "node_sim_start": nsim[0], "node_sim_end": nsim[-1],
            "node_sim_mean": round(float(np.mean(nsim)), 4),
            "gate_open_start": ofrac[0], "gate_open_end": ofrac[-1],
            "gate_open_mean": round(float(np.mean(ofrac)), 4),
            "collapse_mag_mean": round(float(np.mean(cmag)), 4),
            "steps": fires["steps"],
...
def main():
    ...
    ap.add_argument("--diff-gain", type=float, required=True, help="measured-window diff_gain (0.0 or 1.0)")
    ...
    with open(a.out, "w", encoding="utf-8") as fh:
        for s in seeds:
            summ = run_one(s, a.diff_gain, a.warm, a.curriculum,
                           a.warmup_reps, a.warmup_target, fh)
            summaries.append(summ)
            print(f"seed={s:>6} diff_gain={a.diff_gain} phi {summ['phi_start']:.2f}->{summ['phi_end']:.2f} "
                  ...
                  f"nsim={summ['node_sim_mean']:.3f} gate={summ['gate_open_mean']:.3f} "
