...
- **저렴히 확증된 진단 2건** (fable 발견 → 로컬 코드 확인):
  ...
  2. **`too_ordered`는 warm에서 침묵** — 게이트는 좌절도<0.45에서만 발화하나 warm 실측 0.526–0.532.
     ...
     ⚠️ **정정 (A/B 실측으로 반증)**: 애초 여기 적었던 "`too_frustrated`(>0.55)도 미발화 ⇒ warm은
     불감대에 앉아 **무조절**"은 **틀렸다**. 0.526–0.532는 *평균*이고, 셀별로는 **매 스텝 ~10.5/48
     세포가 `too_frustrated` 평활화를 발화**한다(any-rate 1.000, 양팔 동일). warm은 무조절이 아니라
     **상시 수축 조절 중**이었고, 이 평활화가 바로 반발 토크를 매 스텝 되돌린 상쇄 주범이다.
     (부수 정정: 창 시작 시점 PR은 ≈1.03이 아니라 1.006–1.012.)
